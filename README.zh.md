...
## 🎯 为什么选择 OAAI Model-Radar？

你是否厌倦了臃肿复杂的 API 网关？是否担心把珍贵的 API Key 填入不知名的在线测速网站会导致泄漏？OAAI Model-Radar 专为解决这些痛点而生：

| 特性与痛点 | 传统网关环境 | 在线测速台 | 🦀 OAAI Model-Radar |
| :--- | :--- | :--- | :--- |
| **配置复杂度** | 极高 (多半需配置数据库/Redis等) | 无 | **极低 (纯本地，双击即用)** |
| **数据安全性** | 中 (托管到个人服务器) | 低 (上传至他人云端) | **极高 (100% 数据留存本地)** |
| **运行依赖** | 重 (动辄数百 MB 依赖包) | 无 | **零依赖 (仅需 Python 标准库)** |
| **并发测速** | 通常不支持或需写脚本 | 有一定限制 | **支持一键并发测试所有节点** |
